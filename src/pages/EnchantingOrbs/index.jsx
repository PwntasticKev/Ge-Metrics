...
import { enchantingRecipes } from '../../components/Table/data/enchanting-orbs-filters.jsx'
import { useFavorites } from '../../hooks/useFavorites.js'
import GraphModal from '../../shared/modals/graph-modal.jsx'
import PremiumPageWrapper from '../../components/PremiumPageWrapper'

export default function EnchantingOrbs () {
  const { items, mapStatus, priceStatus } = ItemData()
  const [lastFetchTime, setLastFetchTime] = useState(new Date())
  const [currentTime, setCurrentTime] = useState(new Date())
  const [orbSets, setOrbSets] = useState([])
      ...
      {priceStatus === 'success' && items.length > 0 && (
        <Box sx={{ py: 4 }}>
          <Group position="apart" mb="md">
            <div>
              <Text size="xl" weight={700} color="white">Enchantment Spells Profit</Text>
              <Text size="sm" color="rgba(255, 255, 255, 0.7)">
                Monitor profit from all enchantment spells including orbs, jewellery, and crossbow bolts.
                <br />
                Includes Charge Orb spells, Jewellery Enchantments (Lvl-1 through Lvl-7), and Enchant Crossbow Bolt spells.
              </Text>
            </div>
            <Group spacing="md">
              <Badge
              ...
              <Badge
                color={orbSets.length > 0 ? 'green' : 'orange'}
                size="lg"
              >
                {orbSets.length} Items Tracked
              </Badge>
            </Group>
          </Group>

          {priceStatus === 'success' && orbSets && orbSets.length > 0 && (
            <ItemSetsTable
              data={orbSets}
              items={items}
              favoriteItems={favoriteItemIds}
              onToggleFavorite={(itemId) => toggleFavorite(itemId, 'orb')}
